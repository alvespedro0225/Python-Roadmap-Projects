...
class LengthForm(forms.Form):
    ...
    fromUnit = forms.ChoiceField(
        label="From Unit", widget=forms.Select, choices=CHOICES
    )
    toUnit = forms.ChoiceField(label="To Unit", widget=forms.Select, choices=CHOICES)
    unitValue = forms.FloatField(
        label="Value", min_value=0, max_value=9999999999999, required=True
    )
...
class TemperatureForm(forms.Form):
    ...
    MINIMUM_TEMPERATURES = {"Celsius": -273.0, "Fahrenheit": -459.67, "Kelvin": 0.0}
    fromUnit = forms.ChoiceField(
        label="From Unit", widget=forms.Select, choices=CHOICES
    )
    toUnit = forms.ChoiceField(label="To Unit", widget=forms.Select, choices=CHOICES)
    ...
    def clean(self):
        ...
        if value < self.MINIMUM_TEMPERATURES[unit]:
            raise ValidationError(
                f"{unit}'s temperature cannot go bellow {self.MINIMUM_TEMPERATURES[unit]}"
            )
