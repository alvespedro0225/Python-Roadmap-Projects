...
class TemperatureView(View):
    ...
    def fahrenheitAndKelvin(self, value:float, unit:str) -> float:
        if unit == "Fahrenheit":
            ...
            first_mult:float = np.multiply(first_sub, np.reciprocal(self.FAHRENHEIT_CONSTANT))
    ...
    def post(self, request):
        ...
        if previousUnit != newUnit:
            match (previousUnit, newUnit):
                case ("Celsius", "Fahrenheit"): newValue: float = self.fahrenheitAndCelsius(previousValue, previousUnit)
                case ("Celsius", "Kelvin"): newValue: float = self.kevinAndCelsius(previousValue, previousUnit)
                case ("Fahrenheit", "Celsius"): newValue: float = self.fahrenheitAndCelsius(previousValue, previousUnit)
                case ("Fahrenheit", "Kelvin"): newValue:float = self.fahrenheitAndKelvin(previousValue, previousUnit)
                case ("Kelvin", "Celsius"): newValue: float = self.kevinAndCelsius(previousValue, previousUnit)
                case ("Kelven", "Fahrenheit"): newValue: float = np.reciprocal(self.fahrenheitAndKelvin(previousValue, previousUnit))
                case _: raise ValidationError("Unknown unit")
        else:
...
class LenghtView(View):
    ...
    def post(self, request) -> HttpResponse:
        ...
        newUnit:str = data["toUnit"]
        previousValue = float(data["unitValue"])
        valueInMeter:float = np.multiply(previousValue, np.reciprocal(self.UNITS[previousUnit]))
        newValue:float = np.multiply(valueInMeter, self.UNITS[newUnit])
        notationValue: str = np.format_float_scientific(newValue, 5)
        
        ctx:dict[str,float|str] = {"previousValue":previousValue, "previousUnit":previousUnit, "newValue":notationValue, "newUnit":newUnit }
